...
void monster_b::attack_special(player &target_plyaer){
...
bool monster_b::attach_target(player* target_player) {
  if (calculate_distance(*target_player) > 30) {
    this->target_player = target_player;
    cout << "Finding with Location Tracking" << endl;
    return true;
  }
...
void monster_c::attack_special(player &target_player){
  cout << "Strong thunder attack : damage -100hp\n";
}

void monster_c::find_route(player &target_player){
  cout << "Go to the target - Don't be seen by the target\n";
}

bool monster_c::attach_target(player* target_player) {
  this->target_player = target_player;
  cout << "If it stands out, I'll follow it no matter what\n";
  return true;
}
