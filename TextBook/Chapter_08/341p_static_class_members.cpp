...
my_vector::my_vector(int size) : vector_size(size){
    for(int i = 0; i < vector_size; i++) {contents.push_back(0);};
}
...
void linear_algebra::matrix_multiply(matrix &operans1, matrix &operand2){
    cout << "Calculate the product of two matrices.\n";
}

void linear_algebra::matrix_vector_multiply(matrix &operand1, my_vector &operand2){
    cout << "Calculates the product of a matrix and a vector.\n";
}

void linear_algebra::dot_product(my_vector &opoerand1, my_vector &operand2){
    cout << "Calculate the dot product of the vectors.\n";
}
...
void do_matrix_vector_multiply(matrix &matrix_1, my_vector &vectpr_1){
    // 다음 코드는 접근 지정자에 따라 접근할 수 없음
    //linear_algebra::matrix_vector_multiply(matrix_1, vector_1);
}
