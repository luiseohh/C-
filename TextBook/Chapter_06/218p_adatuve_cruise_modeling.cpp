...
#else
    Sleep(accelerarion_adjusing_period / 1000);
#endif
    keep_cruise = false;        
    }
}
...
int main(int argc, char* *argv){
    gs_engine my_engine;
    sensor my_sensor;
    break_system my_braek_system;
    accelerator my_accelerator(my_engine);
    cruise_controller my_cruise_controller(my_sensor, my_accelerator, my_braek_system);
    car my_car(my_engine,my_sensor,my_braek_system,my_accelerator,my_cruise_controller);
    my_car.do_cruise();
